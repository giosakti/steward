...
import {createWorkspace, listWorkspaces, showWorkspace, useWorkspace, showAgent, configureAgent, type CreateWorkspaceInput, type ConfigureAgentInput} from '../workspaces/workspaces.js';
...
async function run(fn: (db: Kysely<Database>) => Promise<unknown>) {
  ...
  try { console.log(JSON.stringify(await fn(db), null, 2)); }
...
workspace.command('create <slug> <name>').description('Create a workspace and its root agent')
  ...
  .action(async (slug: string, name: string, options: Pick<CreateWorkspaceInput, 'description' | 'rootPath'>) => {
...
workspace.command('show [slug]').description('Show a workspace or the current selection')
  .action(async (slug?: string) => { await run(db => showWorkspace(db, slug ?? selected())); });
workspace.command('use <slug>').description('Persist the default workspace for this database')
  .action(async (slug: string) => { await run(db => useWorkspace(db, slug)); });
const agent = program.command('agent').description('Inspect and configure the root agent');
agent.command('show').action(async () => { await run(db => showAgent(db, selected())); });
