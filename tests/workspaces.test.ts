...
async function setup(fn: (pool: pg.Pool, url: string, db: Kysely<Database>) => Promise<void>) {
  await isolated(async (url, _client, schema) => {
    ...
    try { await fn(pool, url, db); } finally { await db.destroy(); await pool.end(); }
  });
}
...
await test('workspaces persist across CLI processes and explicit overrides do not change selection', async () => {
  await setup(async (pool, url) => {
    ...
    await assert.rejects(command(['agent', 'configure']), /Provide --name, --title, or --role-description/);
    await assert.rejects(command(['agent', 'configure', '--title', ' ']), /Agent title must not be blank/);
...
await test('workspace/root-agent constraints prevent duplicates, missing roots, and cross-workspace roots', async () => {
  await setup(async (pool, _url, db) => {
    ...
    const second = await createWorkspace(db, {slug: 'second', name: 'Second'});
    await assert.rejects(createWorkspace(db, {slug: 'first', name: 'Duplicate'}), /unique/);
...
await test('invalid input and audit failures leave workspace and agent state unchanged', async () => {
  await setup(async (pool, _url, db) => {
    await assert.rejects(createWorkspace(db, {slug: 'Invalid Slug', name: 'Invalid'}), /slug/);
    await assert.rejects(createWorkspace(db, {slug: 'blank', name: ' '}), /blank/);
    await assert.rejects(createWorkspace(db, {slug: 'missing', name: 'Missing', rootPath: `/tmp/${randomUUID()}`}), /ENOENT/);
    ...
    await assert.rejects(createWorkspace(db, {slug: 'extra', name: 'Extra', unexpected: true}));
    const workspace = await createWorkspace(db, {slug: 'valid', name: 'Valid'});
    await configureAgent(db, {name: '  Builder  ', roleDescription: 'Original'}, 'valid');
    ...
    await pool.query("ALTER TABLE events ADD CONSTRAINT reject_configuration CHECK (type <> 'AGENT_CONFIGURED') NOT VALID");
    await assert.rejects(configureAgent(db, {title: 'Changed'}, 'valid'), /reject_configuration/);
    ...
    await assert.rejects(createWorkspace(db, {slug: 'failed', name: 'Failed'}), /reject_creation/);
